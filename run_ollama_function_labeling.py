#!/usr/bin/env python3
"""
Run Ollama-based function labeling over prepared mention CSV rows.

Requires a local Ollama server:
  ollama serve

Usage example:
python run_ollama_function_labeling.py \
  --input-csv analysis_ai_refs/llm_labeling_human_mentions.csv \
  --output-csv analysis_ai_refs/llm_labeling_human_mentions_labeled_pilot.csv \
  --model llama3.1 \
  --limit 50
"""
...
def build_user_prompt(row: pd.Series, max_chars: int, tail_chars: int, no_rationale: bool) -> str:
    ...
    return (
        ...
        f"confidence, contains_multiple_functions, rationale. {rationale_line}\n\n"
